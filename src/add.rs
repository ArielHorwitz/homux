use crate::files::{get_relative_path, copy_file_mode};
...
pub fn add(args: AddArgs) -> Result<()> {
    let target_file = if args.target_file.as_path().is_absolute() {
        ...
        std::env::current_dir().context("get current working directory from environment")?.join(&args.target_file)
    };
    let relative_target_file = get_relative_path(&args.target_base, &target_file)
